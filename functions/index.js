...
// Adsterra ad generation function
function generateAdsterraAd(adKey = '213fdc3e1b3f9258d1950caae29e5874', height = 250, width = 300, customClass = '') {
  return `
    <div class="ad-container adsterra-ad ${customClass}" style="margin: 1rem auto; text-align: center; min-height: ${height}px; display: flex; justify-content: center; align-items: center; background: #f9f9f9; border-radius: 8px; overflow: hidden;">
        ...
        <script type="text/javascript" src="//demolitionzerostomach.com/${adKey}/invoke.js"></script>
    </div>
  `;
}
...
function generateHomepageHTML(data) {
  ...
  // FIXED: Generate proper canonical URL for category/search pages
  let canonicalUrl = baseUrl + '/';
  if (categoryFilter) {
    canonicalUrl = `${baseUrl}/?category=${encodeURIComponent(categoryFilter)}`;
  } else if (searchQuery) {
...
<head>
    ...
    <script type="application/ld+json">
    {
        ...
        "inLanguage": "rw",
        "countryOfOrigin": "RW",
        "numberOfItems": ${isSearchOrFilter ? filteredVideos.length : allVideos.length},
        "mainEntityOfPage": {
            "@type": "WebPage",
            "@id": "${canonicalUrl}"
        }
    ...
    <script type="application/ld+json">
    {
        ...
        "itemListElement": [
            ...
            ${searchQuery ? `,
            {
                "@type": "ListItem",
                "position": 2,
                "name": "Search: ${escapeHTML(searchQuery)}",
                "item": "${canonicalUrl}"
            }` : ''}
        ]
    }
...
<div style="width: 300px; height: 250px; background: #ccc; margin: 20px auto;">
    ...
    <main class="container" role="main">
        ...
        ${isSearchOrFilter ? `
            ...
            <div class="search-results">
                ...
                <!-- AD 2: Search Results Top -->
                ${generateAdsterraAd('213fdc3e1b3f9258d1950caae29e5874', 250, 300)}

                ${filteredVideos.length > 0 ? `
                    <div class="videos-grid">
                        ${filteredVideos.map((video, index) => `
                            ${generateVideoCard(video, baseUrl)}
                            <!-- AD 3,4: After every 2 videos in search results -->
                            ${(index + 1) % 2 === 0 && index < filteredVideos.length - 1 ? generateAdsterraAd('213fdc3e1b3f9258d1950caae29e5874', 250, 300) : ''}
                        `).join('')}
                    </div>

                    <!-- AD 5: Search Results Bottom -->
                    ${generateAdsterraAd('213fdc3e1b3f9258d1950caae29e5874', 250, 300)}
                ` : `
        ...
        ` : `
            ...
            ${Object.entries(latestByCategory).map(([category, videos], categoryIndex) => videos.length > 0 ? `
                <section class="category-section" id="${category}">
                    ...
                    <!-- AD 8,11,14,17: Before each category section (every odd category) -->
                    ${categoryIndex % 2 === 0 ? generateAdsterraAd('213fdc3e1b3f9258d1950caae29e5874', 250, 300) : ''}

                    <div class="videos-grid">
                        ${videos.map((video, index) => `
                            ${generateVideoCard(video, baseUrl)}
                            <!-- AD 9,12,15,18: After every 2 videos in each category -->
                            ${(index + 1) % 2 === 0 && index < videos.length - 1 ? generateAdsterraAd('213fdc3e1b3f9258d1950caae29e5874', 250, 300) : ''}
                        `).join('')}
                    </div>

                    <!-- AD 10,13,16: After each category section -->
                    ${generateAdsterraAd('213fdc3e1b3f9258d1950caae29e5874', 250, 300)}
                </section>
            ` : '').join('')}
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const adContainers = document.querySelectorAll('.adsterra-ad');
            
            const observer = new IntersectionObserver((entries) => {
                entries.forEach(entry => {
                    if (entry.isIntersecting) {
                        const adContainer = entry.target;
                        // Ad scripts are already loaded, they should initialize automatically
